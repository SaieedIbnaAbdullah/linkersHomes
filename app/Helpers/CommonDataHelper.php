...
trait CommonDataHelper
{
    public function shareRegistrationData()
    {
        ...
        $data_entry = Registration::whereIn('id', $ids)->where('type', 2)->count();
        array_push($card_array, [
            'title' => 'Data Entry Registration',
            'count' => $data_entry,
            'icon' => 'user-circle-o',
            'type' => 'primary',
            'role' => [0, 2]
        ...
        $online_earning = Registration::whereIn('id', $ids)->where('type', 0)->where('approved', 1)->sum('amount');
        array_push($card_array, [
        ...
        $offline_earning = Registration::whereIn('id', $ids)->where('type', 1)->where('approved', 1)->sum('amount');
        array_push($card_array, [
        ...
        $data_entry_earning = Registration::whereIn('id', $ids)->where('type', 2)->where('approved', 1)->sum('amount');
        array_push($card_array, [
            'title' => 'Data Entry Earning',
            'count' => $data_entry_earning,
            'icon' => 'credit-card-alt',
            'type' => 'primary',
            'role' => [0, 2]
        ]);
        $pending_earning = Registration::whereIn('id', $ids)->where('type', 2)->where('approved', 0)->sum('amount');
        array_push($card_array, [
