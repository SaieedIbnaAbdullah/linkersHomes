...
{
    use UploadHelper, NotificationHelper, BkashHelper;
    ...
    {
        $reg_check = Registration::where('data', 'LIKE', '%"player_mobile":"' . $request->player_mobile . '"%')->first();
        ...
        $request->validate([
            'player_mobile' => 'required|unique:users,email'
        ]);
